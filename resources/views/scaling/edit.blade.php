...
@section('title', 'Edit Scale Sheet #' . $truckLoad->scale_sheet_no . ' - RMD Corp')

@section('content')
<div class="space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <a href="{{ route('scaling.show', $truckLoad->id) }}" class="text-xs text-amber-400 hover:text-amber-300 font-semibold flex items-center gap-1.5 mb-2">
                <i class="fa-solid fa-arrow-left"></i> Back to Scale Sheet
            </a>
        ...
        <div class="text-right">
            ...
            <p class="font-mono text-xl font-bold text-amber-400">{{ $truckLoad->invoice_no }}</p>
        </div>
    </div>

    <form action="{{ route('scaling.update', $truckLoad->id) }}" method="POST" id="scaleEditForm" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="glass-panel p-6 rounded-2xl border border-slate-800 shadow-xl space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                <div>
                    ...
                    <input type="text" value="{{ $truckLoad->supplier->name ?? 'N/A' }}" readonly class="w-full bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-3 text-amber-400 font-bold text-sm focus:outline-none cursor-not-allowed">
                </div>
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider mb-1.5 text-slate-400">Truck Plate No.</label>
                    <input type="text" value="{{ $truckLoad->truck_plate_no }}" readonly class="w-full bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:outline-none cursor-not-allowed">
                </div>
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider mb-1.5 text-slate-400">Scaled By</label>
                    <input type="text" value="{{ $truckLoad->scaled_by }}" readonly class="w-full bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:outline-none cursor-not-allowed">
                </div>
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider mb-1.5 text-slate-400">Date Scaled</label>
                    <input type="text" value="{{ \Carbon\Carbon::parse($truckLoad->date_scaled)->format('M d, Y') }}" readonly class="w-full bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:outline-none cursor-not-allowed">
                </div>

                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider mb-1.5 text-slate-400">Date Unloaded</label>
                    <input type="text" value="{{ \Carbon\Carbon::parse($truckLoad->date_unload)->format('M d, Y') }}" readonly class="w-full bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:outline-none cursor-not-allowed">
                </div>
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider mb-1.5 text-slate-400">Gross Amount</label>
                    <input type="text" id="grossAmountDisplay" value="₱ {{ number_format($truckLoad->gross_amount, 2) }}" readonly class="w-full bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-3 text-emerald-300 text-sm focus:outline-none cursor-not-allowed font-mono">
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-5 glass-panel p-6 rounded-2xl border border-slate-800 shadow-xl flex flex-col justify-between space-y-6">
            <div>
                ...
                <div class="space-y-3 mt-4 text-sm text-slate-300">
                    <div class="flex justify-between items-center border-b border-slate-800 pb-3">
                        ...
                        <span class="font-mono font-bold text-white text-base" id="summaryGrossVal">₱ {{ number_format($truckLoad->gross_amount, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center text-rose-400">
                        <span>Less: Total Deductions</span>
                        <span class="font-mono font-bold text-lg" id="summaryDeductions">- ₱ {{ number_format($truckLoad->total_deductions, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center text-emerald-400">
                        <span>Add Driver's Assistance</span>
                        <span class="font-mono font-bold text-lg" id="summaryDriverAssistance">+ ₱ {{ number_format($truckLoad->drivers_assistance, 2) }}</span>
                    </div>
                    <div class="border-t border-slate-800 pt-3 flex justify-between items-center font-semibold text-white text-base">
                        <span>Net Amount Payable</span>
                        <span class="font-mono text-emerald-300" id="summaryNetPayable">₱ {{ number_format($truckLoad->net_payable, 2) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-3 pt-4">
                ...
                <a href="{{ route('scaling.show', $truckLoad->id) }}" class="text-center text-xs uppercase tracking-[0.3em] text-slate-400 hover:text-slate-200">Cancel and return to invoice</a>
            </div>
        </div>
    </form>
</div>
...
<script>
    function recalculateDeductions() {
        const grossAmount = parseFloat({{ $truckLoad->gross_amount }});
